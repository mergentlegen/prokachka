...
async function call(supabase: { rpc: (name: string, args: Record<string, unknown>) => PromiseLike<{ data: unknown; error: { message?: string; code?: string } | null }> }, name: string, args: Record<string, unknown>): Promise<ReadyAttemptResult> {
  ...
  return result.error ? { error: result.error } : { data: (result.data || {}) as Record<string, unknown> };
}
...
export function answerReadyProgramAttempt(userId: string, taskId: string, answer: number): Promise<ReadyAttemptResult> {
  const supabase = getSupabaseAdmin();
  if (!supabase) return Promise.resolve({ unavailable: true });
  return call(supabase, "app_answer_ready_program", { p_user_id: userId, p_task_id: taskId, p_answer: answer });
}
