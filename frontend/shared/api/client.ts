...
export function mapProgram(row: ApiRow): TaskProgram {
  return { id: String(row.id), teamId: String(row.team_id), title: String(row.title || ""), deadlineHours: Number(row.deadline_hours || 72), isActive: Boolean(row.is_active), isPinned: Boolean(row.is_pinned), publisherId: row.publisher_id ? String(row.publisher_id) : undefined, audienceRootId: row.audience_root_id ? String(row.audience_root_id) : undefined, templateKey: isReadyProgramKey(row.template_key) ? row.template_key : undefined, createdAt: String(row.created_at || new Date().toISOString()), updatedAt: String(row.updated_at || row.created_at || new Date().toISOString()) };
}
