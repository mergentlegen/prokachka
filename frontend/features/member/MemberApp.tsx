...
export function MemberApp() {
  ...
  async function createInviteLink() {
    ...
    try {
      ...
      setToast("Ссылка скопирована. Она бессрочная и требует подтверждения наставника.");
    } catch {
  ...
  return <main className="app-shell">
    ...
    <div className="page-width content-grid"><section className="main-column"><AnnouncementsBlock announcements={store.announcements} /><MemberNetwork users={networkUsers} currentUserId={user.id} /><div className="section-heading"><div><p className="eyebrow">Практика</p><h2>Актуальные задания</h2></div><div className="task-switch"><button className={taskView === "regular" ? "active" : ""} onClick={() => setTaskView("regular")}>Обычные</button><button className={taskView === "programs" ? "active" : ""} onClick={() => setTaskView("programs")}>Программы</button></div><span className="task-count">{visibleTasks.length} заданий</span></div>{visibleTasks.length === 0 ? <EmptyState text="Пока нет активных заданий." /> : <div className="task-list">{visibleTasks.map((task, index) => {
    ...
    })}</div>}<TelegramConnect telegramId={user.telegramId} busy={telegramBusy} onLink={linkTelegram} onRefresh={checkTelegram} /></section><aside className="side-column"><div className="section-heading ranking-heading"><div className="ranking-title"><div><p className="eyebrow">Команда</p><h2>Рейтинг</h2></div><span className="trophy">♛</span></div><div className="rating-switch"><button className={ratingType === "points" ? "active" : ""} onClick={() => setRatingType("points")}>Баллы</button><button className={ratingType === "stars" ? "active" : ""} onClick={() => setRatingType("stars")}>Звёзды</button></div></div>{ranking.length === 0 ? <EmptyState text="Рейтинг пока пуст." /> : <div className={"ranking-list " + (ratingType === "stars" ? "stars-ranking-list" : "")}>{ranking.map((member, index) => <div className={`rank-row ${member.id === user.id ? "current" : ""}`} key={member.id}><span className={`rank-position rank-${index + 1}`}>{index + 1}</span><span className="rank-avatar">{initials(member.name)}</span><span className="rank-name">{member.name}{member.id === user.id && <small>это вы</small>}</span><strong>{member.points}{ratingType === "stars" ? " ★" : ""}</strong></div>)}</div>}<div className="ranking-footer">{ratingType === "stars" ? "Звёзды выдаёт наставник вручную" : "Баллы начисляются после проверки наставником"}</div></aside></div>
    <nav className="bottom-nav"><button className={tab === "home" ? "active" : ""} onClick={() => setTab("home")}><span>⌂</span>Задания</button><button className={tab === "profile" ? "active" : ""} onClick={() => setTab("profile")}><span>◌</span>Профиль</button></nav>
    {tab === "profile" && <ProfileModal user={user} store={store} rank={currentRank} points={currentPoints} stars={store.starAwards.filter((award) => award.userId === user.id).reduce((sum, award) => sum + award.stars, 0)} telegramBusy={telegramBusy} onLinkTelegram={linkTelegram} onRefreshTelegram={checkTelegram} inviteUrl={inviteUrl} inviteBusy={inviteBusy} onCreateInvite={createInviteLink} onClose={() => setTab("home")} />}
...
function ProfileModal({ user, store, rank, points, stars, telegramBusy, onLinkTelegram, onRefreshTelegram, inviteUrl, inviteBusy, onCreateInvite, onClose }: { user: User; store: Store; rank: number; points: number; stars: number; telegramBusy: boolean; onLinkTelegram: () => void; onRefreshTelegram: () => void; inviteUrl: string; inviteBusy: boolean; onCreateInvite: () => void; onClose: () => void }) {
