...
export function MemberApp() {
  ...
  return <main className={`app-shell member-shell member-tab-${tab}`}>
    ...
    <div className="page-width content-grid"><section className="main-column"><div className="member-section member-section-announcements"><AnnouncementsBlock announcements={store.announcements} /></div><div className="member-section member-section-network"><MemberNetwork users={networkUsers} currentUserId={user.id} /></div><div className="member-section member-section-tasks"><div className="section-heading"><div><p className="eyebrow">Практика</p><h2>Актуальные задания</h2></div><div className="task-switch"><button className={taskView === "regular" ? "active" : ""} onClick={() => setTaskView("regular")}>Обычные</button><button className={taskView === "programs" ? "active" : ""} onClick={() => setTaskView("programs")}>Программы</button></div><span className="task-count">{visibleTasks.length} заданий</span></div>{visibleTasks.length === 0 ? <EmptyState text="Пока нет активных заданий." /> : <div className="task-list">{visibleTasks.map((task, index) => {
    ...
    <nav className="bottom-nav"><button className={tab === "home" ? "active" : ""} onClick={() => setTab("home")}><span>⌂</span>Обзор</button><button className={tab === "tasks" ? "active" : ""} onClick={() => setTab("tasks")}><span>☷</span>Задания</button><button className={tab === "ranking" ? "active" : ""} onClick={() => setTab("ranking")}><span>♛</span>Рейтинг</button><button className={tab === "network" ? "active" : ""} onClick={() => setTab("network")}><span>⌘</span>Сеть</button><button className={tab === "profile" ? "active" : ""} onClick={() => setTab("profile")}><span>◌</span>Профиль</button></nav>
