...
export function MemberApp() {
  ...
  return <main className="app-shell">
    ...
    <div className="page-width content-grid"><section className="main-column"><AnnouncementsBlock announcements={store.announcements} /><div className="section-heading"><div><p className="eyebrow">Практика</p><h2>Актуальные задания</h2></div><div className="task-switch"><button className={taskView === "regular" ? "active" : ""} onClick={() => setTaskView("regular")}>Обычные</button><button className={taskView === "programs" ? "active" : ""} onClick={() => setTaskView("programs")}>Программы</button></div><span className="task-count">{visibleTasks.length} заданий</span></div>{visibleTasks.length === 0 ? <EmptyState text="Пока нет активных заданий." /> : <div className="task-list">{visibleTasks.map((task, index) => {
    ...
    {tab === "profile" && <ProfileModal user={user} store={store} rank={currentRank} points={currentPoints} stars={store.starAwards.filter((award) => award.userId === user.id).reduce((sum, award) => sum + award.stars, 0)} telegramBusy={telegramBusy} onLinkTelegram={linkTelegram} onRefreshTelegram={checkTelegram} onClose={() => setTab("home")} />}
...
function ProfileModal({ user, store, rank, points, stars, telegramBusy, onLinkTelegram, onRefreshTelegram, onClose }: { user: User; store: Store; rank: number; points: number; stars: number; telegramBusy: boolean; onLinkTelegram: () => void; onRefreshTelegram: () => void; onClose: () => void }) {
