...
export function AdminApp() {
  ...
  return <main className="admin-shell">
    ...
    <div className="admin-layout">
      ...
      <section className="admin-content"><div className="admin-heading"><div><p className="eyebrow">Панель наставника</p><h1>{section === "dashboard" ? "Добрый день" : section === "tasks" ? "Задания" : section === "review" ? "Проверка работ" : section === "history" ? "История проверок" : section === "announcements" ? "Объявления" : section === "programs" ? "Программы" : section === "stars" ? "Звёзды" : "Заявки в команду"}</h1></div><div className="admin-heading-actions">{section === "tasks" && <button className="primary-button" onClick={() => openTaskModal()}>+ Создать задание</button>}{section !== "requests" && <button className="text-button request-shortcut" onClick={() => setSection("requests")}>Заявки {pendingRequests.length > 0 && "(" + pendingRequests.length + ")"}</button>}</div></div>
