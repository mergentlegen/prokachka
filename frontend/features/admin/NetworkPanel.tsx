...
export function NetworkPanel({ authUser, onError }: { authUser: AuthUser; onError: (message: string) => void }) {
  ...
  async function save(id: string, input: { parentUserId?: string | null; canReview?: boolean; canPublishTasks?: boolean; canInviteMembers?: boolean }) {
    setBusyId(id);
  ...
  return <div className="network-page">
    <div className="admin-panel network-intro"><div><p className="eyebrow">Иерархия команды</p><h2>Структура сети</h2><p>Участник видит свой узел и всех приглашённых ниже него. Права проверки, публикации и приглашений выдаются отдельно.</p></div><button className="primary-button" onClick={() => { void createInvite(); }}>{inviteUrl ? "Создать новую ссылку" : "Создать ссылку"}</button></div>
    {inviteUrl && <div className="admin-panel network-invite"><strong>Многоразовая ссылка готова</strong><input readOnly value={inviteUrl} onFocus={(event) => event.currentTarget.select()} /><button className="button button-edit" onClick={() => void navigator.clipboard?.writeText(inviteUrl)}>Копировать</button><small>Ссылка действует 30 дней и требует подтверждения наставником.</small></div>}
