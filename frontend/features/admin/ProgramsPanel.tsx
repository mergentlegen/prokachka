...
import { createAdminProgram, updateAdminProgram } from "@/frontend/shared/api/admin-client";
...
export function ProgramsPanel({ programs, tasks, onChange, onError }: { programs: TaskProgram[]; tasks: Task[]; onChange: (programs: TaskProgram[], tasks: Task[]) => void; onError: (message: string) => void }) {
  ...
  return <div className="programs-panel">
    ...
    <div className="admin-panel table-panel"><div className="panel-title"><div><p className="eyebrow">Опубликованные</p><h2>Программы команды</h2></div></div>{programs.length === 0 ? <div className="empty-admin"><p>Программ пока нет.</p></div> : programs.map((program) => <div className="program-row" key={program.id}><div><strong>{program.title}</strong><span>{tasks.filter((task) => task.programId === program.id).length} шагов · {program.deadlineHours} ч на шаг</span></div><span className={"admin-status " + (program.isActive ? "active" : "inactive")}>{program.isActive ? "Активна" : "Скрыта"}</span><button className={"button " + (program.isActive ? "button-warning" : "button-success")} onClick={() => { void toggle(program); }}>{program.isActive ? "Скрыть" : "Активировать"}</button></div>)}</div>
  </div>;
}
